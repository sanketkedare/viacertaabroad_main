...
const Form = ({ setFormOpen }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!formData.name || !formData.email || !formData.mobile || !formData.qualification || formData.selectedCountry.length === 0) {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="bg-white p-4 lg:p-10 rounded-2xl shadow-2xl  max-h-full overflow-y-auto w-screen lg:w-9/12">
        <div className="flex justify-between items-center mb-5">
          <h2 className="lg:text-3xl text-xl font-extrabold text-[#152347]">Apply for Scholarship</h2>
          <X className="cursor-pointer text-gray-500 hover:text-[#152347]" size={28} onClick={() => setFormOpen(false)} />
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex items-center border border-gray-300 rounded-xl p-3">
            <FaUser className="text-[#152347] mr-3" />
            <input type="text" name="name" placeholder="Name" value={formData.name} onChange={handleChange} className="w-full focus:outline-none" />
          </div>
          ...
          <div className="flex items-center border border-gray-300 rounded-xl p-3">
            ...
            <input type="text" name="mobile" placeholder="Phone Number" value={formData.mobile} onChange={handleChange} className="w-full focus:outline-none" />
          </div>

          <div className="flex items-center border border-gray-300 rounded-xl p-3">
            <FaGraduationCap className="text-[#152347] mr-3" />
            <select name="qualification" value={formData.qualification} onChange={handleChange} className="w-full focus:outline-none">
          ...
          <div className="flex flex-wrap gap-4 items-center lg:justify-evenly justify-center border border-gray-300 rounded-xl p-3">
            {countries.map((country) => (
              <button
                key={country.name}
                type="button"
                className={`flex items-center gap-2 p-2 rounded-xl transition-all duration-200 ${formData.selectedCountry.includes(country.name) ? "bg-[#152347] text-white" : "bg-gray-200"}`}
          ...
          <button type="submit" className="w-full py-4 bg-[#152347] text-white rounded-xl hover:bg-[#1c2b57] transition duration-200 font-bold" disabled={loading}>
            {loading ? "Submitting..." : "Submit Application"}
          </button>

          {message && (
            <div className={`mt-5 text-center text-lg ${message.startsWith("✅") ? "text-green-600" : "text-red-600"}`}>
              {message}
            </div>
          )}
        </form>
